...
def main(year, stats_output_dir):
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        for i in range(0, len(config.modis_metric_names)):
            ...
            vds_array, vds_transform = resample_raster(
                mds, vds
            )  # Use resample_raster to warp viirs metrics to modis grid
        ...
        for i in range(0, len(config.modis_metric_names)):
            ...
            if "viirs" in d.keys():
                ...
                symlog_norm = SymLogNorm(
                    linthresh=1, vmin=dif.min(), vmax=dif.max(), base=10
                )

                im = ax.imshow(
                    dif, cmap=cmap, norm=symlog_norm, interpolation="nearest"
                )
                ax.set_title(f"{config.modis_metric_names[i]}")
                ax.axis("off")
        ...
        for i in range(0, len(config.modis_metric_names)):
            ...
            if "viirs" in d.keys():
                ...
                indices = np.random.choice(
                    len(modis_valid), size=sample_size, replace=False
                )
        ...
        plt.suptitle(
            f"{year}: MODIS - VIIRS Metrics (sample of {sample_size} pixels)",
            fontsize=14,
        )
        plt.tight_layout(rect=[0, 0, 1, 0.97])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate MODIS/VIIRS metric comparison plots."
    )
    parser.add_argument(
        "year",
        type=int,
        nargs="?",
        default=2019,
        help="Year to process (default: 2019)",
    )
    parser.add_argument(
        "--output_dir",
        "-o",
